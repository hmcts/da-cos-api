...
OAUTH2_CLIENT_SECRET=$(az keyvault secret show --vault-name da-aat -o tsv --query value --name idam-secret)
...
for user in $users; do
  ...
  code=$(curl --insecure --fail --show-error --silent -X POST --user "$email:$password" "${IDAM_API_BASE_URL}/oauth2/authorize?redirect_uri=${REDIRECT_URI}&response_type=code&client_id=da" -d "" | docker run --rm --interactive stedolan/jq -r .code)
  idamToken=$(curl --insecure --fail --show-error --silent -X POST -H "Content-Type: application/x-www-form-urlencoded" --user "da:${OAUTH2_CLIENT_SECRET}" "${IDAM_API_BASE_URL}/oauth2/token?code=${code}&redirect_uri=${REDIRECT_URI}&grant_type=authorization_code" -d "" | docker run --rm --interactive stedolan/jq -r .access_token)
  ...
  caseId=$(curl --insecure --fail --show-error --silent -X GET ${CCD_BASE_URL}/citizens/$userId/jurisdictions/DOMESTICABUSE/case-types/FL401/cases -H "Authorization: Bearer $idamToken" -H "Content-Type: application/json" -H "ServiceAuthorization: Bearer $serviceToken" | docker run --rm --interactive stedolan/jq '.[0].id')
  ...
  curl --insecure --fail --show-error --silent -X POST http://ccd-data-store-api-aat.service.core-compute-aat.internal/citizens/$userId/jurisdictions/DOMESTICABUSE/case-types/FL401/cases/$caseId/events -H "Authorization: Bearer $idamToken" -H "Content-Type: application/json" -H "ServiceAuthorization: Bearer $serviceToken" -d '{"event_token" : "'${eventToken}'","data":{"applicant1FirstName": "'${firstName}'","applicant1LastName":"'${lastName}'","applicant1Email" : "'${email}'"},"event" :{"id" :"patchCase"}}'
done
